...
- фиксированные `Task`, `Project`, `Note`, `CalendarEvent`, `ReflectionEntry` и `ReadingItem` — в роли универсальных объектов;
...
- отдельные заметки и записи осмысления — в документы рабочего пространства с происхождением и правами.
...
### Этап B. Универсальное объектное ядро — первый совместимый срез выполнен
...
7. ✅ Схема v13 сохраняет нативные объекты и связи; повреждённые или будущие данные не перезаписываются молча.
8. ✅ Совместимый адаптер не создаёт вторую редактируемую копию прежних данных.

Рубеж подтвердил жизнеспособность самой рискованной части идеи — универсального фрактального ядра. Визуально и интерактивно проверены desktop, планшет и телефон; автоматический контур содержит 119 тестов в 20 файлах.
...
- добавлен обязательный нативный объектный граф в `DashboardState` v13;
...
Следующая сторона проекта — этап D: полноценный блочный редактор и рабочее пространство. Нужны мультимодальные блоки, файлы, удобная структура, полнотекстовый поиск, история операций и двусторонний обмен с Obsidian. После этого GTD можно постепенно перевести с совместимого адаптера на единое каноническое ядро, не ломая текущую рабочую систему.
