...
Текущая схема `DashboardState` v14 добавляет восстанавливаемую корзину и ограниченную историю изменений задач, событий, заметок и нативных универсальных объектов. Повторные правки одного объекта в течение пяти минут объединяются, поэтому история не превращается в журнал каждого нажатия клавиши; хранится не более 200 точек возврата.
...
Раздел «Осмысление» делает сохранение наблюдаемым: новая запись остаётся в хронологии, одновременно создаёт связанную заметку и открывает её как документ единого рабочего пространства. На главном экране под блоком остаются последние записи, поэтому начало новой больше не выглядит как исчезновение предыдущей. Чтение одной записи при разборе и чтение всей хронологии разведены: общий архив попадает в файловый снимок Codex только после отдельного переключателя в «Осмыслении» или «Интеграциях». Отдельной обязательной сущности «Дневник» в целевой модели нет.
